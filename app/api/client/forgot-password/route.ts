...
export async function POST(request: NextRequest) {
  try {
    ...
    if (dbAvailable) {
      ...
      client =
        mockClients.find((c: { email: string }) => c.email === email) ?? null;
    }
    ...
    const token = await createPasswordResetToken(email, "client");
...
<p>Вы запросили восстановление пароля для входа в личный кабинет.</p>
