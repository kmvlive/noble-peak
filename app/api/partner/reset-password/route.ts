...
export async function POST(request: NextRequest) {
  try {
    ...
    const { hashPassword } = await import("@/lib/partner-auth");
    const { getPartnerByEmail } = await import("@/lib/models");

    const partner = await getPartnerByEmail(resetRecord.email);
    if (!partner) {
      return NextResponse.json(
